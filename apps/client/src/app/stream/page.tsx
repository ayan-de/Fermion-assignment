...
export default function StreamPage() {
            ...
            await new Promise<mediasoupClient.types.TransportOptions>(
              (resolve) => {
                socket.emit("createWebRtcTransport", null, (info: any) => {
                  if (info.error) {
                    console.error(
                      "Error creating producer transport:",
                      info.error
                    );
            ...
            await new Promise<mediasoupClient.types.TransportOptions>(
              (resolve) => {
                socket.emit("createConsumerTransport", null, (info: any) => {
                  if (info.error) {
                    console.error(
                      "Error creating consumer transport:",
                      info.error
                    );
                    setConnectionStatus(`Error: ${info.error}`);
                    return;
  ...
  return (
    <div className="p-4 flex flex-col">
      ...
      <h2 className="mb-1 text-lg font-semibold text-white">🟢 Local Stream</h2>
      <video
        ref={localVideo}
        autoPlay
        muted
